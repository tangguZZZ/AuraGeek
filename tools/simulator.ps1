param([ValidateSet('build','run','demo','test','stress','stocks')][string]$Action='run')
...
try {
 ...
 if($Action -ne 'build'){
   $arguments=@();if($Action -eq 'demo'){$arguments=@('--official')};if($Action -eq 'test'){$arguments=@('--smoke')};if($Action -eq 'stress'){$arguments=@('--stress')}
   ...
   if($Action -in @('test','stress')){$runArgs.WindowStyle='Hidden'}
   $process=Start-Process @runArgs
   if($Action -in @('test','stress')){$process.WaitForExit();exit $process.ExitCode}
 }
